...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    {
        nSubsidy = 0 * COIN; //No insta Mining
    }else if(nHeight > 15110000)
    {
        nSubsidy= 0;
    }
    else
     {
        // Subsidy is cut in half every 944475 blocks, which will occur approximately every 4 years
         nSubsidy >>= (nHeight / 944475); // CataloniaCoin: 944375 blocks in ~4 years
     }
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
        ...
        if (fTestNet) {
                if (pindexLast->nHeight+1 >= 2116) { DiffMode = 2; } // cataloniacoin, 100 blocks after first legacy diff adjustment
        }
        else {         
        	if (pindexLast->nHeight+1 >= 26754) { DiffMode = 2; }  //cataloniacoin, 5 days after 27/01/2014 12:00 UTC
        }
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        //hashGenesisBlock = uint256("0xf5ae71e26c74beacc88382716aced69cddf3dffff24f384e1808905e0188f68f");
          hashGenesisBlock = uint256("0xf70ab3ade8806547550556731553fbd0212198ced256dd242680b9e3ee554bb8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
       ...
       // block.nNonce = 5749262;
        block.nNonce = 11845503;
        block.nTime = 1394550306;
        ...
        printf("=== FOUD BLOCK + GENESIS INFO ===\n");
        block.print();
        printf("hash: %s\n", hash.ToString().c_str());
        printf("hashGenesisBlock:%s\n", hashGenesisBlock.ToString().c_str());
        printf("hashMerkleRooot:%s\n", block.hashMerkleRoot.ToString().c_str());
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Cataloniacoin: increase each by adding 1 to bitcoin's value.
