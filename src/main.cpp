...
uint256 hashGenesisBlock("0xd050f3b1e6e5747163f319e5c4097b338f8bbaafa483d4166a422903f5f99aa3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Cataloniacoin: starting difficulty is 1 / 2^12
...
int64 nChainStartTime = 1385000000; // Line: 2815
...
const string strMessageMagic = "Cataloniacoin Signed Message:\n";
...
{
    // Cataloniacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if (nHeight == 1)
    {
        nSubsidy = 7555000 * COIN; //Premine for the people of catalonia
    }
    else if (nHeight > 1 && nHeight <= 16)
    {
        nSubsidy = 1 * COIN; //No insta Mining
	
    }    
    // Subsidy is cut in half every 236109 blocks, which will occur approximately every 1 year
             nSubsidy >>= (nHeight / 236109); // CataloniaCoin: 236109 blocks in ~1 years
...
static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // CataloniaCoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // CataloniaCoin: 2 minutes
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Cataloniacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cataloniacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cataloniacoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        pchMessageStart[3] = 0xdc;
          ...
          hashGenesisBlock = uint256("0x746d01a4142aff1f5aa43dde2abd9a00bb2d0fbff0b1b3bc8c3112ddc45a1686");
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "30/03/2014 Labour will take axe to student fee";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nNonce = 21499468;
        block.nTime = 1396274945;
        ...
        if (fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0xf46fe2d312aabf0a91c2d13e6f82ce4371000c14703304bcb3be5ed8798b30ff"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0xf46fe2d312aabf0a91c2d13e6f82ce4371000c14703304bcb3be5ed8798b30ff"));        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xd5, 0xdb }; // Cataloniacoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CataloniacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CataloniacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CataloniacoinMiner(CWallet *pwallet)
{
    printf("CataloniacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("cataloniacoin-miner");
    ...
    try { loop {
        ...
        printf("Running CataloniacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CataloniacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CataloniacoinMiner, pwallet));
}
